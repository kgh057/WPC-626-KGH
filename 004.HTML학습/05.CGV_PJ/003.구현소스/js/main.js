...
링크.forEach((el, idx, arr) => {
  // el - 각 요소, idx - 순서, arr - html컬렉션 유사배열
  ...
  el.addEventListener("click", () => {
    ...
    // (2) 포스터메뉴 a링크의 부모인 li에 on 넣기
    // parentElement: a링크의 부모인 li
    el.parentElement.classList.add("on");
  }); // addEventListener() //////

  // 만약 onclick 이벤트 속성으로 셋팅하게 되면 이전에 셋팅된 onclick 이벤트 속성값이 지워진다
  // 왜? 한 요소 안에 한개의 동일 이벤트 속성만 있기 때문
  // el.onclick = () => {
  //   // 2-3. 포스터메뉴 a링크의 부모인 li에 on 넣기
  //   // parentElement: a링크의 부모인 li
  //   el.parentElement.classList.add("on");
  // };
...
// 닫기 버튼 .cbtn을 클릭시 .gmap에 클래스 on 제거하기
